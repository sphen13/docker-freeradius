...
RUN ln -snf /usr/share/zoneinfo/$TZ /etc/localtime && echo $TZ > /etc/timezone \
    && apk add --no-cache talloc ca-certificates bash libwbclient gdbm tzdata tar libpcap mariadb-connector-c postgresql-libs libpq libldap \
    && apk add --no-cache --virtual .build-deps talloc-dev alpine-sdk git linux-headers openssl-dev ldb-dev openldap-dev ruby gdbm-dev mariadb-connector-c-dev \
